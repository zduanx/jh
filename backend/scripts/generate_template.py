...
def generate_template_yaml(sam_config, env_metadata, static_env_vars):
    ...
    yaml_lines.extend([
        ...
        "        - S3CrudPolicy:",
        "            BucketName: !Ref ResumeContentBucket",
        "        - Version: '2012-10-17'",
        "          Statement:",
        "            - Effect: Allow",
        "              Action:",
        ...
        "              Resource:",
        f'                - !Sub arn:aws:logs:${{AWS::Region}}:${{AWS::AccountId}}:log-group:/aws/lambda/{worker_name}:*',
        f'                - !Sub arn:aws:logs:${{AWS::Region}}:${{AWS::AccountId}}:log-group:/aws/lambda/{crawler_name}:*',
        f'                - !Sub arn:aws:logs:${{AWS::Region}}:${{AWS::AccountId}}:log-group:/aws/lambda/{extractor_name}:*',
        "      Environment:",
        "        Variables:",
        f"          WORKER_FUNCTION_NAME: !Ref {worker_name}",
        "          RESUME_CONTENT_BUCKET: !Ref ResumeContentBucket",
        "      Events:",
        "        HttpApiEvent:",
        "          Type: HttpApi",
        "          Properties:",
        ...
        "",
        "  # ==========================================================================",
        "  # MCP Server Lambda - Phase 7B. FastMCP (streamable-http) via Mangum, behind",
        "  # a Function URL (HTTP, no API Gateway). Auth: in-handler service token",
        "  # (MCP_SERVICE_TOKEN). Reuses the backend code/deps/env (it imports db/utils/models).",
        "  # ==========================================================================",
        "  McpServer:",
        "    Type: AWS::Serverless::Function",
        "    Properties:",
        ...
        "",
        "  # ==========================================================================",
        "  # S3 Bucket - Resume storage (user-uploaded PDFs)",
        "  # ==========================================================================",
        "  ResumeContentBucket:",
        "    Type: AWS::S3::Bucket",
        "    Properties:",
        '      BucketName: !Sub "jobhunt-resume-content-${AWS::AccountId}"',
        "      CorsConfiguration:",
        "        CorsRules:",
        "          - AllowedOrigins:",
        '              - "http://localhost:3000"',
    ...
    yaml_lines.extend([
        ...
        "",
        "  McpServerUrl:",
        "    Description: MCP server Function URL (HTTP)",
        "    Value: !GetAtt McpServerUrl.FunctionUrl",
        "",
        "  FunctionName:",
        ...
        "",
        "  ResumeContentBucketName:",
        "    Description: S3 bucket for user-uploaded resumes",
        "    Value: !Ref ResumeContentBucket",
        "",
        "  ExtractorQueueUrl:",
